...
For upcoming events, mention only events whose specific date is explicitly retrieved and is today or later than the current Georgia date included below. An old event page, a recurring event title, or an undated "upcoming" heading does not confirm a future event. If no dated future event is retrieved, say only that you could not confirm a specific upcoming date and direct the visitor to Elachee's official events/calendar page. Do not claim that no events are scheduled, describe old event dates or years, or guess when an event may recur.
...
- For the suggested questions about visits, trails, camps/programs, field trips, events, and hours/admission, answer the whole requested bundle with practical details supported by sources; if only some parts are confirmed, give those parts and clearly mark the rest as unconfirmed. A citation must directly support the topic and details in the answer; unrelated cited pages are not evidence.
...
export function buildSystemInstruction(
  ...
  const languageInstruction =
    language === "en"
      ...
      : language === "es"
      ? "Responde exclusivamente en español claro y natural, aunque el visitante escriba en otro idioma. Conserva sin traducir los nombres oficiales de senderos, lugares y programas; redacta el resto en español. Traduce fielmente los datos recuperados sin agregar detalles. No cambies al inglés a menos que el visitante lo pida."
